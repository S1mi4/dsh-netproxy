		...
		function NetProxyPage() {
			...
			return react.createElement("div", { style: { maxWidth: 720, display: "flex", flexDirection: "column", gap: 14 } },
				...
				react.createElement("div", { style: bannerStyle(error, s) }, error
					...
					: `代理当前：${SOURCE_NAMES[s.source] || s.source}${s.egress ? ` → ${s.egress}` : ""}${s.routeKind === "engine" ? `（经本地引擎 ${s.hot && s.hot.target || ""} 桥接）` : ""}${s.envReady || s.egress ? "" : "（待生效）"}`),
				...
				react.createElement("div", { style: cardStyle() },
					...
					s === null ? null : react.createElement(react.Fragment, {},
						react.createElement(Field, { label: "出口路由" }, s.routeKind === "direct" ? "直连（不经代理）" : s.hot && s.hot.active
							? (s.routeKind === "engine" ? `本地引擎 ${s.hot.target} 桥接 → ${s.egress}` : `直连上游 ${s.hot.target}`)
							: "未生效（直连或待开启）"),
						react.createElement(Field, { label: "本地引擎" }, s.engine.running ? `运行中 · ${s.engine.bind}:${s.engine.port}${s.engine.upstream ? " → " + s.engine.upstream : ""}` : (s.routeKind === "engine" ? "未运行" : "不需要（仅 SOCKS 场景使用）")),
