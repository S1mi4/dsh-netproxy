...
export function apply(ctx, config) {
  ...
  const attachSettings = () => {
    ...
    scope.watch((next) => {
      ...
      state.egress = resolveEgress(cfg);
      updateEnvFacts();
      applyTarget();
    });
  };
  ...
  const state = {
    source: 'direct',
    egress: '',                      // resolved outbound (custom url / system url)
  ...
  /** Resolve the egress URL from the CURRENT config (null = direct). */
  const resolveEgress = (cfg) => {
    if (!cfg || cfg.source === 'direct') return null;
    if (cfg.source === 'custom') {
      const host = String(cfg.customHost || '').trim();
      const port = Number(cfg.customPort) || 0;
      if (!host || !port) return null; // incomplete custom config => direct
      const proto = String(cfg.customProtocol || 'http').toLowerCase();
      if (!PROTOCOLS.includes(proto)) return null;
      const auth = String(cfg.customAuth || '').trim();
      return `${proto}://${auth ? `${auth}@` : ''}${host}:${port}`;
    }
    if (sysRes.url) return sysRes.url;
  ...
  const applyTarget = () => {
    ...
    maybeReloadCa();
    const egress = resolveEgress(cfg);
    state.source = cfg.source;
    state.egress = egress || '';
    ...
    if (!isRunning() || state.engine.port !== port || state.engine.bind !== bind || state.engine.upstream !== egress) {
      stopEngine();
      startEngine();
    }
  ...
  const startEngine = () => {
    const cfg = readConfig();
    const egress = resolveEgress(cfg);
    if (!egress || !cfg.source || cfg.source === 'direct') return;
    if (isRunning()) return;
    const port = cfg.enginePort || 4317;
    const bind = cfg.engineBind || '127.0.0.1';
    ...
    spawned.on('exit', (code, signal) => {
      ...
      if (cfgNow.restartOnCrash && resolveEgress(cfgNow)) {
  ...
  const updateEnvFacts = () => {
    ...
    if (cfg.source === 'direct') {
      ...
      state.envReady = true;
      state.envHint = `${state.hot.target || ''} 出口已生效（→ ${state.egress}，热切换无需重启）。${useEnvProxy ? '' : '（建议启动时带 --use-env-proxy，以覆盖子进程/工作线程）'}`;
    } else if (cfg.source === 'system' && !state.egress) {
      state.envReady = false;
  ...
  const buildState = async () => {
    ...
    updateEnvFacts();
    state.source = cfg.source;
    state.egress = resolveEgress(cfg) || state.egress;
    state.skipVerify = !!cfg.skipVerify;
  ...
  state.egress = resolveEgress(cfg0) || '';
  updateEnvFacts();
